...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        ...
        self.conv1 = conv3x3(in_channels, out_channels, kernel_size=1, stride=stride, padding=0)#Resnet50 中，从第二个残差块开始每个layer的第一个残差块都需要一次downsample
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=10):
        ...
        self.layer3 = self.make_layer(block, 128, 256, layers[2], 2)
        self.layer4 = self.make_layer(block, 256, 512, layers[3], 2)
        self.avg_pool = nn.AvgPool2d(3,stride=1,padding=1)
        self.fc = nn.Linear(math.ceil(img_height/32)*math.ceil(img_width/32)*512, num_classes)
...
#Resnet-18 2-2-2-2 总计(2+2+2+2)*2=16 个conv层 加上开头的2个conv 一共18层
